...
class QwenCommandParser:
    ...
    def parse_command(self, user_command):
        ...
        if place_in_container_result:
            print("🔧 Using: place in container handler")
    ...
    # -----------------------------------------------------------------------
    # Handler: place in container (e.g. "pick bowl 3 and place in drawer")
    # -----------------------------------------------------------------------
    def _detect_and_handle_pick_and_place_in_container(self, command):
        ...
        for friendly_name, obj_id in FRIENDLY_NAME_TO_OBJ_ID.items():
            ...
            if normalized_friendly in command_lower:
                ...
                pos = ARTICULATIONS[articulation]["placement_coords"]["center"]
                return [
                    # 1. Open container first (hands free)
                    ...
                    {"type": "navigate", "target": articulation},
                    {
                        ...
                        "subtask_config": {"obj_goal_thresh": 0.5},  # ← 50cm sphere instead of 15cm
                    },
                    # 4. Close container (hands free again)
                    {"type": "navigate", "target": articulation},
                    {
                        "type": "close",
                        "articulation_type":                    ARTICULATIONS[articulation]["articulation_type"],
    ...
    def _detect_and_handle_pick_and_place(self, command):
        ...
        for friendly_name, obj_id in FRIENDLY_NAME_TO_OBJ_ID.items():
            ...
            if normalized_friendly in command_lower:
                ...
                pos = [COUNTER_X, COUNTER_Y, COUNTER_Z]
...
5. Always close drawer/fridge after picking or placing inside it

Example - Pick from counter:
[
...
[
  ...
  {{"type": "navigate", "target": "013_apple-0"}},
...
[
  ...
  {{"type": "open", "articulation": "drawer", "obj_id": "024_bowl-0"}},
  {{"type": "navigate", "target": "drawer"}},
  {{"type": "place", "obj_id": "024_bowl-0", "target": "drawer"}},
  {{"type": "navigate", "target": "drawer"}},
