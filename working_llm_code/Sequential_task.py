...
class SequentialTaskEnv(SceneManipulationEnv):
    ...
    place_cfg = PlaceSubtaskConfig(
        horizon=200,
        obj_goal_thresh=0.33, #Rafael you changed this from 0.15 to 0.30
        ee_rest_thresh=10, #Rafael you changed this from 0.05 to 10
        robot_resting_qpos_tolerance=10.0,          #Rafael you added this line
        robot_resting_qpos_tolerance_grasping=10.0, #Rafael you added this line
    )
    ...
    horizon=800,
    ee_rest_thresh=10.0, #from 5 to 10
    navigated_successfully_dist=6, # from 3
    )
    # The OG Verions down below
    # navigate_cfg = NavigateSubtaskConfig(
    #     horizon=500,
    #     ee_rest_thresh=0.05,
    ...
    horizon=500,                        # ← increased from 200
    ee_rest_thresh=10.0,                 # ← loosened from 0.05
    joint_qpos_open_thresh_frac=dict(
        default=0.9,
        fridge=0.3,                     # ← lowered from 0.75, only needs to open 40%
        kitchen_counter=0.9,
    ),
    )
    #The OG Verions down below
    ...
    horizon=500,                        # ← increased from 200
    ee_rest_thresh=5.0,                 # ← loosened from 0.05
    ...
    def _load_scene(self, options):
        ...
        for subtask_num, subtask in enumerate(self.tp0.subtasks):
            if isinstance(subtask, PlaceSubtask):
                ...
                goal = self._make_goal(
                    radius=0.1, #Rafael you changed this from 0.05 to 0.1
                    name=f"goal_{subtask_num}",
                    goal_type="sphere",
                )
            else:
